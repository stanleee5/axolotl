...
# logger = logging.getLogger("axolotl.monkeypatch.medusa")
...
def add_medusa_heads(self, medusa_num_heads=4, medusa_num_layers=0):
    """
    Args:
        self (nn.Module): the base language model to be used
        medusa_num_heads (int, optional): Number of heads for the Medusa layer. Default is 4.
        medusa_numlayers: (int, optinoal): Number of ResBlock layesr for each Medusa head. Default to 0
    """
    ...
    # Ensure medusa_head's dtype and devie align with the base_model
    self.medusa_head.to(self.dtype).to(self.device)

    for i in range(medusa_num_heads):
        # Initialize the weights of each medusa_head using the base model's weighs.
...
def replace_compute_loss(
    medusa_heads_coefficient: float = 1.0,
    medusa_decay_coefficient: float = 0.0,
    medusa_scheduler: str = "constant",
    medusa_logging=False,
    medusa_logging_topk=5,
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        """
        Copute the trainig loss for the MedusaModel.

        Args:
            model (torch.nn.Module): the model for which to compute the loss
            inputs (dict): The input data, including input IDs, attention masks, and labels.
        ...
        for i in range(medusa):
            ...
            if i == 0:
                if medusa_self_distillation:
                    original_logits = (
                        original_logits[:, :-1]
                        .contiguous()
                        .view(-1, original_logits.shape[-1])
                    )
            ...
            # add top-k accuracy
            for k in range(1, medusa_logging_topk + 1):
...
def replace_create_optimizer(medusa_lr_multiplier):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            # Seperately set lr for medusa_head
    ...
    def deepspeed_init(trainer, num_training_steps, inference=False):
        ...
        if inference:
            ...
            # important for when deepspeed_init is used as re-init
